...
from studyai.services.web_media import (
    WebMediaError,
    _youtube_video_id,
...
def test_youtube_duration_is_read_from_public_watch_metadata(app, monkeypatch):
    class Response:
        def __enter__(self):
            return self
        ...
        def read(self, _maximum):
            return b'{"videoDetails":{"lengthSeconds":"14752"}}'

    class Opener:
        def open(self, *_args, **_kwargs):
            return Response()

    monkeypatch.setattr("studyai.services.web_media._safe_opener", lambda: Opener())
    with app.app_context():
        assert fetch_youtube_duration("https://youtu.be/4VCq0Xix2EM") == 14752
